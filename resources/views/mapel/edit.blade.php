...
<div class="container mx-auto mt-10 max-w-7xl">
  <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
    <h1 class="text-center font-semibold text-3xl">Form Edit Mapel</h1>
    <form action="{{ route('update-mapel',$mapel->id) }}" method="post" enctype="multipart/form-data">
      @csrf
      <div class="flex flex-col gap-2">
        <label for="">Kode Mapel</label>
        <input type="text" name="kode" class="p-2 border rounded-md" value="{{ $mapel->kode }}">
        <span class="text-red-500">{{ $errors->first('kode') }}</span>
      </div>

      <div class="flex flex-col gap-2">
        <label for="">Mata Pelajaran</label>
        <input type="text" name="mapel" class="p-2 border rounded-md" value="{{ $mapel->mapel }}">
        <span class="text-red-500">{{ $errors->first('mapel') }}</span>
      </div>

      <div class="flex flex-col gap-2">
        <label for="">Nama Pengajar</label>
        <input type="text" name="pengajar" class="p-2 border rounded-md" value="{{ $mapel->pengajar }}">
        <span class="text-red-500">{{ $errors->first('pengajar') }}</span>
      </div>

      <div class="flex flex-col gap-2">
        <label for="">Deskripsi</label>
        <textarea name="deskripsi" id="" cols="30" rows="5" class="border rounded-md">{{ $mapel->deskripsi }}</textarea>
